...
public class MySQLUsuarioDAO implements UsuarioDAO {
    ...
    private static final String TABLA_USUARIOS = "usuarios_20252437";
    ...
    public Usuario iniciarSesion(String nombreUsuario, String password) throws Exception {
        String sql = "SELECT * FROM " + TABLA_USUARIOS + " WHERE usuario = ?";
    ...
    public List<Usuario> listarUsuarios() throws Exception {
        ...
        String sql = "SELECT * FROM " + TABLA_USUARIOS + " ORDER BY id";
        try (Connection connection = DatabaseConnection.getInstance().getConnection();
             PreparedStatement statement = connection.prepareStatement(sql);
             ResultSet result = statement.executeQuery()) {
            while (result.next()) {
    ...
    public void registrar(Usuario usuario, String password) throws Exception {
        String sql = "INSERT INTO " + TABLA_USUARIOS + "(usuario, nombre, apellido, telefono, correo, password_hash) VALUES(?,?,?,?,?,?)";
        try (Connection connection = DatabaseConnection.getInstance().getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    public void actualizar(Usuario usuario, String nuevaPassword) throws Exception {
        ...
        String sql = cambiarPassword
                ? "UPDATE " + TABLA_USUARIOS + " SET usuario=?, nombre=?, apellido=?, telefono=?, correo=?, password_hash=? WHERE id=?"
                : "UPDATE " + TABLA_USUARIOS + " SET usuario=?, nombre=?, apellido=?, telefono=?, correo=? WHERE id=?";
        try (Connection connection = DatabaseConnection.getInstance().getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setString(1, usuario.getUsuario());
            statement.setString(2, usuario.getNombre());
    ...
    public void eliminar(int id) throws Exception {
        try (Connection connection = DatabaseConnection.getInstance().getConnection();
             PreparedStatement statement = connection.prepareStatement("DELETE FROM " + TABLA_USUARIOS + " WHERE id=?")) {
